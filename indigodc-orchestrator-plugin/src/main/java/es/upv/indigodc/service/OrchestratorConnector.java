...
/**
 * Manage and do the calls to the REST API exposed by the IndigoDC Orchestrator; 
 * Connect to the authorization system to obtain a token for the calls
 * to the IndigoDC Orchestrator
 * 
 * @author asalic
 */
@Slf4j
...
public class OrchestratorConnector {
  ...
  /**
   * Store the token information received after a successful call to the user authorization 
   * service from DEEP
   * @author asalic
   */
  @Data
  ...
  /**
   * Obtain the list of deployments created by the the user with the client id stored in 
   * the cloud configuration for the plugin
   * @param cloudConfiguration The configuration used for the plugin instance
  ...
  /**
   * Deploy an alien 4 cloud topology
   * It is already adapted to the normative TOSCA supported by the Orchestrator
   * @param cloudConfiguration The configuration used for the plugin instance
   * @param yamlTopology The actual topology accepted by the orchestrator.
   * It is a string formated and packed for the orchestrator e.g. new lines
   * are replaced with their representation of '\n'.
  ...
  /**
   * General method that actually performs the calls of the DEEP Orchestrator and authorization endpoints 
   * It can connect to both HTTPS and HTTP servers
  ...
  public OrchestratorResponse restCall(URL requestURL, String postData, 
      Map<String, String> headers, boolean isSecured, SSLContext sslContext, int requestType) throws IOException, 
      NoSuchFieldException, OrchestratorIAMException {   
  ...
  /**
   * Obtain the response from the DEEP endpoint once a successful HTTP/HTTPS connection is established
  ...
  public static String getOrchestratorUUIDDeployment(OrchestratorResponse response) throws JsonProcessingException, IOException {
