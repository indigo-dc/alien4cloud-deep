...
public class IndigoDcOrchestrator implements IOrchestratorPlugin<CloudConfiguration> {
  ...
  public static final String DEFAULT_NOT_SET_OUTPUT_VALUE = "<not_set>";
  public static final String TYPE = "IndigoDC";

  protected static final Pattern NOT_FOUND_MATCHER = Pattern.compile("(?=.*not)(?=.*found)");
  ...
  //
  @Getter protected String orchestratorId;
  ...
  public void deploy(PaaSTopologyDeploymentContext deploymentContext, IPaaSCallback<?> callback) {
    CloudConfiguration configuration =
        cloudConfigurationManager.getCloudConfiguration(
    ...
    try {
      ...
      OrchestratorResponse response =
          orchestratorConnector.callDeploy(
              configuration.getOrchestratorEndpoint(), yamlPaasTopology);
      ...
      mappingService.registerDeployment(
          new DeploymentInfo(
              a4cUuidDeployment,
      ...
      // statusManager.addStatus(deploymentContext, orchestratorUuidDeployment,
      // DeploymentStatus.DEPLOYMENT_IN_PROGRESS);
      ...
      if (deploymentInfo != null) {
        ...
        final OrchestratorResponse response =
            orchestratorConnector.callUndeploy(
                configuration.getOrchestratorEndpoint(), orchestratorUuidDeployment);
        log.info(response.toString());
        //        mappingService.registerDeploymentInfo(orchestratorUuidDeployment,
        // a4cUuidDeployment,
        //            deploymentContext.getDeployment().getOrchestratorId(),
        ...
        log.info(
            String.format(
                "Deployment with ID %s not found in the list of deployments",
      ...
      switch (er.getHttpCode()) {
        case 404:
          if (NOT_FOUND_MATCHER.matcher(er.getTitle().toLowerCase()).find()) {
            event.setDeploymentStatus(DeploymentStatus.UNDEPLOYED);
  ...
  public void getInstancesInformation(
    ...
    final DeploymentInfo di =
        mappingService.getByA4CDeploymentPaasId(deploymentContext.getDeploymentPaaSId());
    ...
    if (di != null) {
        ...
        if (di.getOrchestratorDeploymentId() != null) {
          ...
          OrchestratorResponse response =
              orchestratorConnector.callDeploymentStatus(
                  configuration.getOrchestratorEndpoint(), di.getOrchestratorDeploymentId());
          log.info(response.toString());
          ...
          Util.InstanceStatusInfo instanceStatusInfo =
              Util.indigoDcStatusToInstanceStatus(
                  response.getStatusTopologyDeployment().toUpperCase());
      ...
      //         if (deploymentInfo != null) {
      //           final String groupId = deploymentContext.getDeploymentPaaSId();
      //           final Map<String, Map<String, InstanceInformation>> topologyInfo = new
      // HashMap<>();
      //           final Map<String, String> runtimeProps = new HashMap<>();
      //           final Map<String, InstanceInformation> instancesInfo = new HashMap<>();
      //           final Map<String, String> outputsVars =
      ...
      //                  .forEach(outputE -> outputsVars.replace(outputE.getKey(),
      // outputE.getValue()));
      //            }
      //            final InstanceInformation instanceInformation = new
      // InstanceInformation(instanceStatusInfo.getState(),
      ...
      ////        } catch (OrchestratorIamException er) {
      ////          final InstanceInformation instanceInformation = new
      // InstanceInformation("UNKNOWN", InstanceStatus.FAILURE,
      ...
      log.warn(
          "Deployment with DeploymentPaaSId "
              + deploymentContext.getDeploymentPaaSId()
  ...
  protected void updateStatusesDeploymentsUser(
    ...
    try {
      ...
      log.info(
          "Get the orchestrators UUIDs for all deployments managed "
              + "by the currently logged in user");
      OrchestratorResponse response =
          orchestratorConnector.callGetDeployments(configuration.getOrchestratorEndpoint());
      log.info(response.getResponse().toString());
      final GetDeploymentsResponse gdr =
          response.<GetDeploymentsResponse>getResponse(GetDeploymentsResponse.class);
      final List<DeploymentOrchestrator> deployments = gdr.getContent();
      final ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
      for (DeploymentOrchestrator deployment : deployments) {
        response =
            orchestratorConnector.callGetTemplate(
                configuration.getOrchestratorEndpoint(), deployment.getUuid());
        log.info(response.toString());
  ...
  protected Map<String, String> extractTopologyOutputs(DeploymentTopology topo) {
    ...
    if (topo.getOutputAttributes() != null) {
      topo.getOutputAttributes()
          .entrySet()
          .stream()
          .forEach(
              outputAttributeEntry ->
                  outputAttributeEntry
                      .getValue()
                      .forEach(
                          outputAttribute ->
                              results.put(
                                  outputAttributeEntry.getKey() + "_" + outputAttribute,
                                  DEFAULT_NOT_SET_OUTPUT_VALUE)));
    }
    if (topo.getOutputCapabilityProperties() != null) {
      topo.getOutputCapabilityProperties()
          .entrySet()
          .stream()
          .forEach(
              outputCapaPropEntry ->
                  outputCapaPropEntry
                      ...
                      .stream()
                      .forEach(
                          outputCapaPropSubEntry ->
                              outputCapaPropSubEntry
                                  .getValue()
                                  .forEach(
                                      outputCapaProp ->
                                          results.put(
                                              outputCapaPropEntry.getKey()
                                                  + "_"
                                                  + outputCapaPropEntry.getKey()
                                                  + "_"
                                                  + outputCapaProp,
              ...
              outputPropertyEntry ->
                  outputPropertyEntry
                      ...
                      .forEach(
                          outputProperty ->
                              results.put(
                                  outputPropertyEntry.getKey() + "_" + outputProperty,
                                  DEFAULT_NOT_SET_OUTPUT_VALUE)));
    }
