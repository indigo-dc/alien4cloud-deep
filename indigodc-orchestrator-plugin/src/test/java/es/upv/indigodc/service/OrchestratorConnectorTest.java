...
public class OrchestratorConnectorTest {
  ...
  public static final String TOKEN_AUTH_RESPONSE = "{\"expires_in\": 1000000, \"access_token\": \"none\"}";
  ...
  public void setup() throws Exception {
      testServer = new TestServer(18080, 18443, 
...
//      servlets.put("/", new TestBlockingServlet());
  ...
  public void deployUndeployProductionAuthSingleToscaCompute() throws JsonParseException, JsonMappingException, IOException, NoSuchFieldException {
	...
	URL url = OrchestratorConnectorTest.class.getClassLoader().getResource("test_compute_indigodc.yaml");
  ...
  public void testLoginWithTestServer() throws Exception {
    ...
    servlets.put("/token", new TestBlockingServlet(TestBlockingServlet.CONTENT_TYPE_JSON, 200, TOKEN_AUTH_RESPONSE));
    servlets.put(ORCHESTRATOR_COMMON_PATH + OrchestratorConnector.WS_PATH_DEPLOYMENTS, new TestBlockingServlet(TestBlockingServlet.CONTENT_TYPE_JSON, 200, 
  ...
  @Ignore("Not testing method")
  protected CloudConfiguration getTestConfiguration() throws JsonParseException, JsonMappingException, IOException {
    ObjectMapper mapper = new ObjectMapper();
    // Be sure that the configuration contains the right certificate stored in the keystore, since it is for localhost and unsigned
...
//    cf.setTokenEndpoint(v.replaceAll("(http|https)(://)(.+?)(/)*", String.format("%s://%s:%d/", "https", "localhost", testServer.getSecurePort())));
//    v = cf.getIamHost();
//    cf.setIamHost(v.replaceAll("(http|https)(://)(.)+(/)*", String.format("%s://%s:%d/", "https", "localhost", testServer.getSecurePort())));
//    v = cf.getOrchestratorEndpoint();
//    cf.setOrchestratorEndpoint(v.replaceAll("(http|https)(://)(.)+(/)*", String.format("%s://%s:%d/", "https", "localhost", testServer.getSecurePort())));
    return cf;
  }
